...
import { FilteredStatusType } from '../types/types';
...
function CheckBox({
  ...
  return (
    <div
      onClick={() => setCheckState((prev: boolean) => !prev)}
      ...
      <input
        ...
        checked={checkState}
      />
      <p className="text-slate-700 dark:text-slate-400 dark:group-hover:text-slate-100">{label}</p>
    </div>
  );
}
